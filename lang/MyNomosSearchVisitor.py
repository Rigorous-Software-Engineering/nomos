...
class MyNomosSearchVisitor(NomosVisitor):
    def __init__(self, outfile, beam_size=1, temperature=0.1):
    ...
    def visitImport_(self, ctx: NomosParser.Import_Context):
        ...
        if self.temperature == 0:
            self.outfile.write("%s.temperature = None\n" % (self.dataset))
        else:
            self.outfile.write("%s.temperature = %f\n" % (self.dataset, self.temperature))

        self.outfile.write("print('Beam size and temperature set to %d and %f.')\n" % (self.beam_size, self.temperature))
    ...
    def visitInput_(self, ctx: NomosParser.Input_Context):
        ...
        self.outfile.write("%s_path = sys.argv[4] \n" % var)
        self.outfile.write("\nfr = open(%s_path, 'r')" % var)
        self.outfile.write("\nx1 = str(fr.read())")
        self.outfile.write("\nfr.close()\n")
        self.outfile.write("\n%s.prob_name = %s_path.split('/')[-1].split('.')[0] \n" % (self.dataset, var))

        # moved here in nomos search
    ...
    def visitPostcondition(self, ctx: NomosParser.PostconditionContext):
        ...
        self.outfile.write("print('Prediction time: ' + str(%s.pred_time))" % (self.dataset))
