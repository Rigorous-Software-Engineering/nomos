...
from transpiler.preprocessing.bpe_modes.fast_bpe_mode import FastBPEMode
from transpiler.preprocessing.dictionary import (
    Dictionary,
    BOS_WORD,
    EOS_WORD,
    PAD_WORD,
...
from transpiler.preprocessing.model import build_model
from transpiler.preprocessing.utils import AttrDict, TREE_SITTER_ROOT

SUPPORTED_LANGUAGES = ["cpp", "java", "python", "go", "rust"]
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=str(
            Path(__file__).parents[0].joinpath("preprocessing/data/bpe/cpp-java-python/codes")
        ),
...
class Translator:
    ...
    def translate(
        ...
        with torch.no_grad():
            ...
            if flat: tokens = src_lang_processor.flatten_func(tokens)
