...
class TransformerModel(nn.Module):
    ...
    def generate_beam(
        ...
        while cur_len < global_max_len:
            ...
            # )
            # print(next_words)
            # print(next_words.squeeze(1))
            # print(next_scores)
            # next_words = torch.multinomial(
            #         F.softmax(_scores.float() / 0.5, dim=1), 2 * beam_size
            #     ).squeeze(1)
            # est = _scores[0][next_words[0]].expand_dims
            # print(_scores[0][next_words[0]])
            # print(next_words[0])
            ...
            if sample_temperature is None:
                ...
                next_words = torch.multinomial(
                    F.softmax(_scores.float() / sample_temperature, dim=1), 2 * beam_size
                ).squeeze(1)
                next_scores = _scores[0][next_words[0]]
                next_scores = next_scores[None, :]
