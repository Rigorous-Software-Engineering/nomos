...
while budget > 0:
	x2 = transpiler.chBranchCond(x1,"java")

	if not(x2 != None) :
		transpiler.precond_violtn += 1
		budget -= 1
		pbar.update(1)
		continue
	...
	if not(transpiler.numConditionals(d2,"py") == transpiler.numConditionals(x2,"java")) or transpiler.numConditionals(d1,"py") == transpiler.numConditionals(x1,"java") :
		transpiler.passed += 1
	else:
