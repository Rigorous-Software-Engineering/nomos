...
while budget > 0:
	x2 = transpiler.addConditional(x1,"java")
	d1 = transpiler.transpile(x1, "java", "py")
	d2 = transpiler.transpile(x2, "java", "py")

	if not(transpiler.compiles(d1,"py") and transpiler.compiles(d2,"py")) or ( not ( transpiler.retValues(x2,"java") == transpiler.retValues(d2,"py") ) or transpiler.retValues(x1,"java") == transpiler.retValues(d1,"py") ) :
		transpiler.passed += 1
	else:
