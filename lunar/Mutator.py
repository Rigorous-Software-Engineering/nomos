...
class SeedPolicyMutator(Mutator):
    ...
    def mutate(self, seed, rng):
        ...
        for _ in range(mut_bdgt):
            ...
            # during training vectorized env used 
            try:
                _ = act[0]
            except:
                act = [act]
...
class RandomActionMutator(Mutator):
    ...
    def mutate(self, seed, rng):
        ...
        for _ in range(mut_bdgt):
            ...
            elif self.wrapper.env_iden == "lunar":
                act = rng.choice(self.wrapper.action_space)
            _, nn_state, done = self.wrapper.env_step([act])  # during training vectorized env used 
            if done:
                return None, None
